...
<p style="align:center"><img src="./pngs_redrawn/GradientDescent_1.png" style="zoom:75%; "/></p>
...
在这个例子中，损失函数处于二维平面，相当于参数只有一个，并且我人为的指定其表达式为：
$$
\begin{align}
J &= 2 \theta^2 -4\theta + 2 \\
...
<p style="align:center"><img src="./pngs_redrawn/GradientDescent_2.png" style="zoom:100%; "/></p>



* 橙色的点上，导数是一个负值，此时，$\theta$若减小（往左挪），$J$将变大（往上挪）
* 绿色的点上，导数为0，此时，$\theta$不论左右移动都使得$J$增大。
* 红色的点上，导数是一个正值，此时，$\theta$若减小（往左挪），$J$将变大（往下挪）
...
<p style="align:center"><img src="./pngs_redrawn/GradientDescent_3.png" style="zoom:100%; "/></p>
...
<p style="align:center"><img src="./pngs_redrawn/GradientDescent_4.png" style="zoom:50%; "/></p>

$\eta$太大：

<p style="align:center"><img src="./pngs_redrawn/GradientDescent_5.png" style="zoom:50%; "/></p>

对于梯度下降法，在例子中是一个二次函数，它显然有唯一的极值点，但是并不是所有的函数都有唯一的极值点，很多时候，机器学习使用的梯度下降法面临的函数是很复杂的，比如：

<p style="align:center"><img src="./pngs_redrawn/GradientDescent_6.png" style="zoom:50%; "/></p>
...
可以确定的是$\theta=(\theta_0, \theta_1, \cdots,\theta_n)$是一个向量，内涵的元素都是**未知量**，参考[梯度下降法介绍](#Introduction-Of-Gradient-Descent)中的横坐标，有$(n+1)$个元素，即便是简单线性回归法里，也是一个包含两个元素的向量。
...
<p style="align:center"><img src="./pngs_redrawn/GradientDescent_7.png" style="zoom:75%; "/></p>
...
| ... | ... | ... |    .     | $X_2^n$   |
| $\hat{y}_i$ | $X_i^1$ | $X_i^2$ | $X_i^3$ | $X_i^n$ |
...
回顾梯度下降法求多元线性回归的损失函数的最小值的操作，如果$m$，也就是样本的个数，很大的时候，计算梯度本身也是非常耗时的。
...
每一次都随机取得一个$i$，对$i$进行这个式子的计算。这个式子也是一个向量，也可以表达一个方向，向这个方向进行迭代，获取损失函数的最小值。

搜索过程就像是这样：

<p style="align:center"><img src="./pngs_redrawn/GradientDescent_8.png" style="zoom:25%; "/></p>
...
另外，在随机梯度下降法中，学习率$\eta$的取值也变得很重要，甚至不能是一个固定的值。很有可能随机梯度下降法已经到了最小值附近了，随机的过程不够好，然后学习率又是个固定式，就会导致它有可能跳去最小值的周围。

所以，在随机梯度下降的过程中，学习率应该是递减的。
...
<p style="align:center"><img src="./pngs_redrawn/GradientDescent_9.png" style="zoom:50%; "/></p>
...
<p style="align:center"><img src="./pngs_redrawn/GradientDescent_10.png" style="zoom:50%; "/></p>
...
将BGD，SGD的优点结合的思想在机器学习中是一种常见的思路，只不过是多一些超参数。

另外，在随机梯度下降中提到了随机，事实上，随机在机器学习中是有意义的。

* 跳出局部最优解
* 更快的运行速度
* 随机搜索（网格搜索的随机版），随机森林，蒙特卡洛树等多种算法都需要随机的特点
