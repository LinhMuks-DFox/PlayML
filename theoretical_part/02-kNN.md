...
假设，有一个机器学习的程序，被用来判断一个肿瘤的良恶性。假设肿瘤的良恶性只和肿瘤的生长时间以及肿瘤的大小有关。 已知一组肿瘤数据，包含三个内容，肿瘤的大小，生长时间，以及良性/恶性标签（Label）。

<p style="align:center"><img src="./pngs_redrawn/kNN_1.png" style="zoom:50%; "/></p>
...
<p style="align=center"> <img src="./pngs_redrawn/kNN_2.png" style="zoom:50%; "></img></p>
...
<p style="align=center"> <img src="./pngs_redrawn/kNN_3.png" style="zoom:50%; "></img></p>

再比如，有一个新的样本点

<p style="align=center"> <img src="./pngs_redrawn/kNN_4.png" style="zoom:50%; "></img></p>
...
对于kNN算法来说，其步骤中有一个非常重要的步骤：计算距离，那么如何定义两个样本点之间的距离呢？ 一个简单的方法就是使用**欧几里得距离**

###### <span id="eula-distance"> 欧几里得距离</span>

若，$A=(x_a, y_a), B=(x_b, y_b)$为二维空间中的两个点，则，其欧几里得距离$d$为： 
$$
d(A, B) = \sqrt {
(x_a - x_b)^2 + (y_a - y_b)^2 }
$$
...
若，$A=(x_a, y_a, z_a), B=(x_b, y_b, z_b)$为3维空间中的两个点，则，其欧几里得距离$d$为： 

$$
d(A, B) = \sqrt {
(x_a - x_b)^2 + (y_a - y_b)^2 + (z_a - z_b)^2 }
$$
现在将距离推广到N维空间，欧几里得距离的表示方法为： 
$$
...
$$
也并不是说，必须使用欧几里得距离 事实上，定义空间中两个点的距离的方法有很多，比如曼哈顿距离，闵可夫斯基距离等等，欧几里得距离只是闵可夫斯基距离的一个特例而已
...
\left[ \sum^n_{i=1} |x_i-y_i|^p\right]^{\frac{1}{p}}
$$

事实上，欧几里得距离，曼哈顿距离，都是闵可夫斯基距离的特例，其中$p=2$时为欧几里得距离，$p=1$时为曼哈顿距离
...
当然，可以使用树结构比如KD-Tree，Ball-Tree来进行优化
